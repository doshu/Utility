<?php  
    class Doshu_Bonifico_Model_Standard extends Mage_Payment_Model_Method_Abstract {  
    
  		protected $_code = 'bonifico';
		protected $_canAuthorize = true;

		protected $_formBlockType = 'bonifico/form';
    	protected $_infoBlockType = 'bonifico/info';

		public $sapCode = "T";
		public $expire = 30;
		
		public function getExpire($start) {
			$start->add(new DateInterval('P'.$this->expire.'D'));
			$month = $start->format('m');
			$lastDay = cal_days_in_month(CAL_GREGORIAN, $month, $start->format('Y'));
			$start->setDate($start->format('Y'), $start->format('m'), $lastDay);
			return $start;
		}
		...
		*/
		...
		public function getPaymentUrl($order) {
			...
			$totaleOrdine = str_replace('.', '', number_format($orderData['subtotal_incl_tax'], 2, '.', ''));
			$valuta = 978;
			$flagRiciclaOrdine = 'Y';
			$flagDeposito = 'N';
