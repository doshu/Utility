...
class Mage_Catalog_Model_Product_Type_Price
{
     ...
     * @return float
     */
    public function getFinalPrice($qty = null, $product, $firstCall = true)
    {
        ...
        if ($firstCall && $product->getTierPriceCount() > 0) {
    ...
    protected function _calcConfigProductTierPricing($product)
    {
        ...
        if ($items = $this->_getAllVisibleItems()) {
            ...
            foreach ($items as $item) {
            	if(isset($idQuantities[$item->getSku()]))
            		$idQuantities[$item->getSku()] += $item->getQty();
            	else
            		$idQuantities[$item->getSku()] = $item->getQty();
            }
            
            // compute the total quantity of items of the configurable product
            if(isset($idQuantities[$product->getSku()])) {
                
                $tierPrice = $this->getFinalPrice($idQuantities[$product->getSku()], $product, false);
            }
        }
