...
function doshuupload(opt) {
	...
	this.start = function(additionalData) {
		...
		if(this.supportFormData()) {
			...
			if(this.file.getAttribute('multiple')) {
			    for(fileIndex = 0; fileIndex < this.file.files.length;  fileIndex++) {
			        fd.append(this.filename+"["+fileIndex+"][file]", this.file.files[fileIndex]);
		        }
			}
    		else {
    		    fd.append(this.filename, this.file.files[fileIndex]);
    		}	
			
			if(additionalData) {
				for(data in additionalData) {
					fd.append(data, typeof additionalData[data] == "string"?additionalData[data]:additionalData[data].val());
				}
			}
			...
			if(additionalData) {
				for(data in additionalData) {
				    if(typeof additionalData[data] == "string") {
				        form.append('<input type="hidden" name="'+data+'" value="'+additionalData[data]+'" />');
				    }
				    else {
