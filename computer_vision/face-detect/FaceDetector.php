...
class FaceDetector
{
	...
	private $height;
	private $foundRects;
	public $device = '/dev/video0';
	public $cameraWidth = 320;
	public $cameraHeight = 240;
	...
	public function scanDevice() {
		for($i = 0; $i <= 9; $i++) {
			if(file_exists('/dev/video'.$i)) {
				$this->device = '/dev/video'.$i;
				return;
			}
		}
	}
	
	public function shoot($device = null) {
		$device = is_null($device)?$this->device:$device;
		$data = "";
		ob_start();
		echo system("uvccapture -m -d".$device." -o/dev/stdout -x".$this->cameraWidth." -y".$this->cameraHeight." 2&>/dev/null");
		$data = ob_get_clean();
		return $data;
	}
	
	
	public function scanFromImageData($data)
	{
